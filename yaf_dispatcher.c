...
zend_class_entry *yaf_dispatcher_get_controller(zend_string *app_dir, zend_string *module, zend_string *controller, int def_module) /* {{{ */ {
	...
	if (EXPECTED(directory_len)) {
		...
		//这里根据配置区分前缀模式还是后缀模式,Controller_Index 或者Index_Controller,ControllerIndex或者IndexController.
...
zend_class_entry *yaf_dispatcher_get_action(zend_string *app_dir, yaf_controller_t *controller, char *module, int def_module, zend_string *action) /* {{{ */ {
	...
	if (EXPECTED(IS_ARRAY == Z_TYPE_P(actions_map))) {
		...
		*(action_upper) = toupper(*action_upper);
		//前后缀模式
		//Index_Action或Action_Index
		//IndexAction或ActionIndex
		if (YAF_G(name_suffix)) {
		...
		if ((paction = zend_hash_find(Z_ARRVAL_P(actions_map), action)) != NULL) {
			...
			//把值(后面的文件路径)赋值给action_path也就是得到了action诶所在的文件了
...
int yaf_dispatcher_handle(yaf_dispatcher_t *dispatcher, yaf_request_t *request,  yaf_response_t *response, yaf_view_t *view) /* {{{ */ {
	...
	zend_string *app_dir = YAF_G(directory);//代码根目录.

	request_ce = Z_OBJCE_P(request);
	//代表request开始,如果出现异常,则会在把当前状态改为0,在分发循环while会判断这个值,如果为0则代表之前分发中存在异常所以保证在分发forward_limit次数内
	//在继续分发,如果等于1则代表没有异常情况,分发完毕.
	...
	if (!app_dir) {
		...
		if (UNEXPECTED(Z_TYPE_P(module) != IS_STRING || !Z_STRLEN_P(module))) {
			...
			return 0;
			//判断当前请求的model是否在modules里面,默认情况modules里面也是Index,如果自定义其他的model,就需要添加在modules这个选项里面,添加的无论是神码
			//modules,都需要存在Index这个model,类似于Index,Admin,Main这样,不添加的话会提示错误.
		...
		//判断当前的model是否等于默认的model,如果等于就去默认的controllers目录下找控制器,如果不等于这个值就为0则就去modules/model/controllers/下面去找控制器
		...
		if (!ce) {
			...
			//获取view路径,如果这个is_def_module为1,则取默认的views目录,如果为0,则取modules/model/views这个目录.
			if (is_def_module) {
			...
			//判断在controller对象里面是否有这个action函数,如indexaction
			if ((fptr = zend_hash_find_ptr(&((ce)->function_table), func_name)) != NULL) {
				//Action是一个定义在controller类中的成员方法,就直接调用controller定义的action方法.
				uint count = 0;
				zval *call_args = NULL;
				...
				if (fptr->common.num_args) {
					...
					//如果不存在这个action函数则按照第二种action方式去获取看看在这个controller里面是否存在actions变量,因为有可能在这个actions数组变量里面记录所有的action类方法地址
...
/* }}} */
/*
路由环节的入口,dispatcher初始化时会创建内置路由器,注意---是"路由器"!不是自定义路由协议,而是你可以重写一个路由器,
我们通常在项目中自定义路由协议就可以了,没有必要自己实现一个路由器.而且框架中其实也是写死了内置路由器,没有给你set自定义路由器的接口
*/
int yaf_dispatcher_route(yaf_dispatcher_t *dispatcher, yaf_request_t *request) /* {{{ */ {
	...
	if (EXPECTED(IS_OBJECT == Z_TYPE_P(router))) {
		...
		if (EXPECTED((router_ce = Z_OBJCE_P(router)) == yaf_router_ce)) {
			/* use built-in route *///执行路由器里面的路由协议
			if (yaf_router_route(router, request)) {
				return 1;
			}
		} else {
			/* user custom route */ //自定义路由器.
...
yaf_response_t *yaf_dispatcher_dispatch(yaf_dispatcher_t *dispatcher, zval *response_ptr) /* {{{ */ {
	...
	//在路由结束后，分发循环之前调用各个插件的loop_startup函数 phoenix
	...
	//进入分发循环，最多循环 application.forward_limit 次   phoenix
	do {
		 //在请求分发之前执行各个插件的pre_dispatch方法 phoenix
		YAF_PLUGIN_HANDLE(plugins, YAF_PLUGIN_HOOK_PREDISPATCH, request, response);
		//捕获异常
		YAF_EXCEPTION_HANDLE(dispatcher, request, response);
		 //调用yaf_dispatcher_handle处理请求,即开始分发.     phoenix
		if (!yaf_dispatcher_handle(dispatcher, request, response, view)) {
			YAF_EXCEPTION_HANDLE(dispatcher, request, response);
			zval_ptr_dtor(response);
			return NULL;
	...
	//取出返回给客户端的数据给一个response对象   phoenix
