...
//application  params init. phoenix
...
*/
/*
* 一切构造从new Yaf_application()开始(在public/index.php中有new Yaf_application(),new首先调用构造函数)
*
* 所有类的实例化都从Yaf_application()的构造函数开始,做的事情:
* 1. 实例化 config;
* 2. 实例化 Request;
* 3. 实例化 Dispatcher;
* 4. 实例化 Loader
...
PHP_METHOD(yaf_application, __construct) {
	...
	//ZEND_STRL(_app)-->(_app),(sizeof(_app)-1)
	//app = zend_read_static_property(yaf_applicatioin_ce,(_app),(sizeof(_app)-1),1);
	...
	if (!section || Z_TYPE_P(section) != IS_STRING || !Z_STRLEN_P(section)) {
		...
		//实例化config,用于读取配置信息.phoenix add
		(void)yaf_config_instance(&zconfig, config, &zsection);
		zval_ptr_dtor(&zsection);
	} else {
		//获取config对象.phoenix add
		(void)yaf_config_instance(&zconfig, config, section);
	}
	//yaf_application_parse_option:用于解析application的配置信息,将配置参数保存在全局变量zconfig中. phoenix add
	...
	//2. 实例化Request,保存请求信息. phoenix add
	...
	//3.实例化Dispatcher,这个是yaf的核心控制类.主要在里面调用插件,路由,分发等 phoenix add
	...
	if (YAF_G(local_library)) {
		...
		//获取一个loader加载类,并同时向内核注册一个自动加载器autoload,这里注册自动加载其也是用内核提供的spl_autoload_register
	...
	//yaf_application::_app赋值成当前self对象.
...
*/
/*
* 几乎所有MVC的Web框架处理一个请求都会走过3个流程:
*  0.request--->1.路由(根据request_uri确定处理请求的controller和action)
*	    	--->2.处理请求(调用action定义的函数来处理用户请求)
*		    --->3.返回数据(action返回的数据输出给客户端)
*/
PHP_METHOD(yaf_application, run) {
	...
	//确保application的状态是非running的.phoenix
	...
	//把application的状态置为running.phoenix
	ZVAL_BOOL(running, 1);
	/* zend_update_property(yaf_application_ce, self,
	 * ZEND_STRL(YAF_APPLICATION_PROPERTY_NAME_RUN), running); */
	//调用dispatcher 来处理这次请求.yaf_dispatcher_dispatch是处理请求的中心函数,它调用路由器完成路由,然后根据路由结果,
	//找到对应的action执行,最后把结果发送给客户端.phoenix
...
PHP_METHOD(yaf_application, bootstrap) {
	...
	zend_class_entry  *ce;
	yaf_application_t *self = getThis();
	//判断Bootstrap类是否存在,默认第一次肯定不存在,因为还没有加载注册进来 phoenix add
	if (!(ce = zend_hash_str_find_ptr(EG(class_table),
		...
		if (!yaf_loader_import(bootstrap_path, 0)) {
			...
			retval = 0;
						...
						YAF_DEFAULT_BOOTSTRAP_LOWER, sizeof(YAF_DEFAULT_BOOTSTRAP_LOWER) - 1)) == NULL)) {//获取bootstrap类
			php_error_docref(NULL, E_WARNING, "Couldn't find class %s in %s", YAF_DEFAULT_BOOTSTRAP, ZSTR_VAL(bootstrap_path));
			retval = 0;
		} else if (UNEXPECTED(!instanceof_function(ce, yaf_bootstrap_ce))) {//判断类是否继承Yaf_Bootstrap_Abstract
